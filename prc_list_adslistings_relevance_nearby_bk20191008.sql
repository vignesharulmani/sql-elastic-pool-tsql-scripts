CREATE procedure [dbo].[prc_list_adslistings_relevance_nearby]
@cityid int,                              
@areaid int=0,                              
@subcatid int, 
@addefid INT=0,                             
...
if exists (select top 1 1 from @NearbyLocalityfiltertable)
	insert into @tblarea(areaid,isnearby,radius)
		select areaid,1,0 from @NearbyLocalityfiltertable
     
/*clean temp table to load exact & nearby ads*/
delete from #adid
...
begin
		...
		--and exists (select top 1 1 from citygroupmaster cgm (nolock) where cgm.cityid = a.cityid and cgm.groupcityid = @lv_cityid)
		) a where dup=1   
		) 
		,Ads_need_attribute                
		as                              
...
;With Ads_need                              
		...
		(                              
		select 
			adid,mode,campaignid,oapc,attributevaluecount,attributecount,cityid,isnearby             
		from (
		select 
			row_number() over (partition by a.adid order by a.adid) as dup,
			row_number() over (partition by 
								...
								order by 
									case when isnull(a.campaignid,0) > 0 then 1 else 2 end,
								ta.isnearby,a.mode desc,a.minprice) oapc,
			a.adid,a.mode,a.campaignid,a.cityid,ta.isnearby,
			count(asam.attributevalueid) over (partition by asam.adid) attributevaluecount,
			count(asam.attributeid) over (partition by asam.adid) attributecount
		from dbo.adsNeedMapping(nolock)  a
			join dbo.adssubcatattributemapping (nolock) asam on a.adid = asam.adid                     
		...
		select 
			row_number() over (order by 
					...
					case when isnull(campaignid,0) > 0 then 1 else 2 end,isnearby,
					oapc,mode desc,dbo.fn_get_runrate(0,cityid,0,campaignid) desc
					) as RowID,adid,isnearby
		from (                              
		select 
			row_number() over (partition by an.adid order by an.adid) as dup,
			an.adid,an.oapc,an.mode,an.campaignid,an.attributevaluecount,an.attributecount,an.cityid,an.isnearby
		from Ads_need an 
		) a where a.dup=1 and a.attributecount = @attributecount
		)
...
begin
		...
		select 
			...
			row_number() over (partition by 
									case when isnull(a.campaignid,0) > 0 then campaignid else 1 end 
								order by ta.isnearby,a.mode desc,a.minprice) oapc,
