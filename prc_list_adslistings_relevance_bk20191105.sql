CREATE procedure [dbo].[prc_list_adslistings_relevance_bk20191105]  
@cityid int,                                
@areaid int=0,                                
@subcatid int,   
@addefid INT=0,                               
...
begin  
  ...
  and (a.subcategoryid=@lv_subcatid or @lv_subcatid = 0)   
...
;With Ads_need              
  ...
  select   
   row_number() over (order by   
     case when @applytopslot = 1 and dbo.fn_get_topslot(adid) > 0 then dbo.fn_get_topslot(adid) else 6 end,  
     --case when isnull(campaignid,0) > 0 then 1 else 2 end,  
     oapc,mode desc,dbo.fn_get_runrate(0,cityid,0,campaignid) desc  
     ) as RowID,adid  
  from (                                
...
begin  
  ...
  select top(( @RowsToFetch * @pageno )+ @morerecords)  
   row_number() over (order by   
        --case when isnull(campaignid,0) > 0 then 1 else 2 end,
		oapc,mode desc,dbo.fn_get_runrate(0,cityid,0,campaignid) desc) as RowID,  
   adid  
  from (  
  select   
   row_number() over (partition by a.adid order by a.adid) as dup,  
...
begin  
  ...
  select top(( @RowsToFetch * @pageno )+ @morerecords)  
   row_number() over (order by   
        case when @applytopslot = 1 and dbo.fn_get_topslot(adid) > 0 then dbo.fn_get_topslot(adid) else 6 end,  
        --case when isnull(campaignid,0) > 0 then 1 else 2 end,  
        isnearby,oapc,mode desc,dbo.fn_get_runrate(0,cityid,0,campaignid) desc) as RowID,  
   adid,isnearby    
  from (  
  select   
   row_number() over (partition by a.adid order by a.adid) as dup,  
